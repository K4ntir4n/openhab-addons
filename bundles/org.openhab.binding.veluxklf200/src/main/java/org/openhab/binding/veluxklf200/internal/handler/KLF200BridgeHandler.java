            ...
            if (THING_TYPE_VELUX_BLIND.equals(t.getThingTypeUID())
                ...
                if (node.getCommandStatus() == CommandStatus.COMPLETE) {
                    ...
                    if (node.getNode().getCurrentPosition().isUnknown()) {
                        ...
                        logger.debug("Blind/Shutter '{}' is currentley {}% closed.", node.getNode().getName(),
                                pctClosed);
                        updateState(channelUid, new PercentType(pctClosed));
                    }
                    channelUid = t.getChannel(VELUX_VELOCITY).getUID();
