...
public class KLFCommandProcessor {
    ...
    private boolean setupKLFConnection(String host, int port) {
        try {
            ...
            try {
                ...
                ctx.init(null, new TrustManager[] { TrustAllTrustManager.getInstance() }, null);
            } catch (Exception e) {
                logger.error("Unable to create an SSL context: {}", e.getMessage());
                return false;
            }

            final SSLSocket rawSocket = (SSLSocket) ctx.getSocketFactory().createSocket(host, port);
            if (rawSocket != null) {
                rawSocket.startHandshake();
                klfOutputStream = new DataOutputStream(rawSocket.getOutputStream());
                klfInputStream = new DataInputStream(rawSocket.getInputStream());

                klfRawSocket = rawSocket;
            }
    ...
    public void shutdown() {
        ...
        try {
            ...
            npeSafeClose(klfInputStream);
            npeSafeClose(klfOutputStream);
            npeSafeClose(klfRawSocket);
        } catch (IOException e) {
    ...
    private void npeSafeClose(@Nullable Closeable closable) throws IOException {
        if (closable != null) {
            closable.close();
        }
    }
    ...
    private void processRequest(BaseKLFCommand command) {
        final CopyOnWriteArrayList<BaseKLFCommand> procList = this.processingList;
        final DataOutputStream os = klfOutputStream;

        if (null != os && null != procList) {
            if (command.isValid()) {
    ...
    private BaseKLFCommand findInProgressCommand(short commandCode, byte[] data) {
        ...
        final CopyOnWriteArrayList<BaseKLFCommand> procList = this.processingList;
        if (procList != null) {
            Iterator<BaseKLFCommand> iterator = procList.iterator();
            while (iterator.hasNext()) {
                BaseKLFCommand match = iterator.next();
                if (match.canHandleResponse(commandCode, data)) {
                    logger.trace("Found a matching command for the response code {}",
            ...
            // perform a task)
            if (this.eventNotification != null && !this.eventNotification.isOnWatchList(commandCode)) {
                if (null == result) {
    ...
    private String getQueueContents() {
        final CopyOnWriteArrayList<BaseKLFCommand> procList = this.processingList;
        final LinkedBlockingDeque<BaseKLFCommand> queue = this.commandQueue;
        if (procList == null || queue == null) {
            return "";
        }
        String result = "In Progress: [";
        for (BaseKLFCommand cmd : procList) {
            String part[] = cmd.getClass().getName().split("\\.");
            result += part[part.length - 1];
            result += ", ";
        }
        result += "], Waiting: [";
        for (BaseKLFCommand cmd : queue) {
            String part[] = cmd.getClass().getName().split("\\.");
            result += part[part.length - 1];
            result += ", ";
        }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                while (!queueShutdown) {
                    BaseKLFCommand c;
                    try {
                        final LinkedBlockingDeque<BaseKLFCommand> queue = commandQueue;
                        if (queue != null) {
                            c = queue.take();
                            boolean delayProcessing = false;
                            if ((c.getKLFCommandStructure().isNodeSpecific())
                                    && (!c.getKLFCommandStructure().isSessionRequired())) {
                                // The current command is node specific, but does not utilise a session. As such, there
                                // is a
                                // risk that if there is another command similar to this already executing that we could
                                // end
                                // up with a collision and the KLF200 getting confused by which request to process /
                                // respond
                                // to.
                                if (isSimilarInProgress(c)) {
                                    // There is a similar command in progress, so lets put the current command back on
                                    // the
                                    // queue and wait for a little while to allow the current one to complete
                                    logger.debug(
                                            "Similar node specific command already executing, delaying execution: {}",
                                            c);
                                    queue.putFirst(c);
    ...
    private void handleGeneralError(VeluxErrorResponse error) {
        final CopyOnWriteArrayList<BaseKLFCommand> procList = processingList;
        if (procList != null && procList.size() == 1) {
            // Likely that the offending Command is the only one in the list.
            BaseKLFCommand bad = procList.iterator().next();
            procList.remove(bad);
            ...
            public void run() {
                ...
                while (!queueShutdown) {
                    try {
                        ...
                        final DataInputStream klfInputStream2 = klfInputStream;
                        if (klfInputStream2 != null) {
                            int messageLength = klfInputStream2.read(temp, 0, temp.length);
                            if (messageLength == -1) {
                                ...
                                if ((null != decoded) && (BaseKLFCommand.validateKLFResponse(decoded))) {
                                    ...
                                    if (KLFCommandCodes.GW_ERROR_NTF == command) {
                                        ...
                                        final KLFEventNotification eventNot = eventNotification;
                                        if (eventNot != null && eventNot.isOnWatchList(command)) {
                                            eventNot.notifyEvent(command, decoded);
                                        }
                                        ...
                                        if (null != cmd) {
                                            ...
                                            if (eventNotification != null
                                                    && !eventNotification.isOnWatchList(command)) {
                                                logger.error(
        ...
        if (queue != null) {
            ...
            if (ret) {
                ...
                logger.error("Command {} could not be added to the queue.",
                        c.getKLFCommandStructure().getCommandCode());
            }
            return ret;
        }
        return false;
